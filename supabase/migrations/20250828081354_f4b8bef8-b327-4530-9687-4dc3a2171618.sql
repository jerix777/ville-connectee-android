...
DROP POLICY IF EXISTS "Administrative users can view data access logs" ON public.data_access_logs;
...
CREATE POLICY "Administrative users can view data access logs" 
ON public.data_access_logs 
FOR SELECT 
USING (
  EXISTS (
    ...
    AND ur.role = 'autorite_administrative'
  )
);
