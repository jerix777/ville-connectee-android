...
export function EditExpenseForm({ expense, onSuccess, onCancel }: EditExpenseFormProps) {
  const [formData, setFormData] = useState({
    ...
    date_depense: expense.date_depense ? new Date(expense.date_depense).toISOString().split('T')[0] : '',
    justificatif_url: expense.justificatif_url || ''
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await associationService.updateDepense(expense.id, {
      ...
      toast({
        title: "Erreur",
        description: "Impossible de modifier la dépense. Veuillez réessayer.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
